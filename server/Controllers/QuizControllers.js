...
const genAI = new GoogleGenerativeAI(
  process.env.GEMINI_API_KEY || "AIzaSyCG0ePJM6-MqxblV3HFyDVnbmG-c35O04Q"
);
  ...
  try {
    ...
    for (let i = 0; i < answers.length; i++) {
      ...
      const isCorrect =
        ...
        questionData.question_data.correct_option_index;
...
export const getUserQuizStats = async (req, res) => {
  try {
    ...
    const bestPercentage = Math.max(
      ...quizAttempts.map((attempt) => attempt.percentage)
    );
